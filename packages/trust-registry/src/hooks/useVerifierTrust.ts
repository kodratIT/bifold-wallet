 ...
 * Hook for checking verifier trust status
 */
...
/**
 * Hook to check verifier trust status
 * @param did - The DID of the verifier to check
 * @returns Trust result, loading state, and error
 */
export function useVerifierTrust(did: string | undefined): UseVerifierTrustResult {
  const { isEnabled, checkVerifier } = useTrustRegistry()
  ...
  const fetchTrustStatus = useCallback(async () => {
    ...
    try {
      const result = await checkVerifier(did)
